...
from fedlg.utils.chemutils import random_scaffold_split, is_valid_smiles, canonical, make_smi_attribute, smiles_from_MolBlock
...
class MoleculeNetDataset(InMemoryDataset):
    ...
    def dataset_names(self):
        dataset_names = {
            ...
            'multi_classification': ['tox21', 'clintox', 'sider', 'toxcast']
        }
...
class DrugBankDataset(InMemoryDataset):
    ...
    def ddi_to_datalist(self, drug1_list: list, drug2_list: list, interaction_list: list, list_all_smiles=None):
        data_list = []
        for drug1, drug2, label in tqdm(zip(drug1_list, drug2_list, interaction_list), total=len(interaction_list)):
    ...
    def smiles_to_datadict(self, smiles_list):
        data_dict = {}
        for smi in tqdm(set(smiles_list), total=len(set(smiles_list))):
            smi = canonical(smi)
...
class CoCrystalDataset(InMemoryDataset):
    ...
    def cocrystal_to_datalist(self, coformer_list1: list, coformer_list2: list, interaction_list: list, list_all_smiles=None):
        data_list = []
        for coformer1, coformer2, label in tqdm(zip(coformer_list1, coformer_list2, interaction_list), total=len(interaction_list)):
