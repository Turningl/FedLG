...
import torch
import numpy as np
from fedlg.utils.chemutils import scaffold_split
...
def molecule_dirichlet_distribution(args, dataset, sample_clients, alpha, null_value=-1, seed=1234):
    ...
    Returns:
        dict: A dictionary mapping database indices to their respective data indices.
    """
    ...
    # Initialize a list of empty lists, one for each database.
    ...
    for idx in range(len(scaffold_unique) + 1):
        ...
        # Compute the Dirichlet distribution proportions for each database.
        proportions1 = np.random.dirichlet(np.repeat(alpha, sample_clients))

        # Adjust proportions based on the current size of each database's batch.
        ...
        # Split the indices according to the computed proportions and add them to each database's batch.
        index_batch = [idx_j + idx.tolist() for idx_j, idx in zip(index_batch, np.split(idx_label, proportions))]

    # Shuffle the indices in each database's batch to ensure randomness.
...
def random_distribution(args, dataset, sample_clients, alpha, null_value=-1, seed=1234):
    ...
    batch_idxs = np.array_split(idxs, sample_clients)  # Split the permuted indices into batches for each database.

    scaffold_split_map = {i: batch_idxs[i] for i in range(
        sample_clients)}  # Create a dictionary mapping database indices to their respective data indices.
